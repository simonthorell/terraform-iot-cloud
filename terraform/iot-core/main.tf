...
resource "local_file" "iot_cert_pem" {
  ...
  filename = "${path.root}/../iot-device/.certs/iot_cert.pem"
}
...
resource "local_file" "iot_private_key" {
  ...
  filename = "${path.root}/../iot-device/.certs/iot_private_key.pem"
}

# Download the AWS Root CA certificate using a local-exec provisioner
resource "null_resource" "download_root_ca" {
  provisioner "local-exec" {
    command = "curl -o ${path.root}/../iot-device/.certs/root_ca.pem https://www.amazontrust.com/repository/AmazonRootCA1.pem"
  }

  # Ensure this runs only when necessary (if the file doesn't exist)
  triggers = {
    root_ca_exists = fileexists("${path.root}/../iot-device/.certs/root_ca.pem") ? 1 : 0
  }
}
...
resource "local_file" "iot_root_ca" {
  ...
  content    = file("${path.root}/../iot-device/.certs/root_ca.pem")
  filename   = "${path.root}/../iot-device/.certs/root_ca.pem"
}
...
resource "local_file" "iot_endpoint_file" {
  ...
  filename = "${path.root}/../iot-device/.certs/iot_endpoint.txt"
}
